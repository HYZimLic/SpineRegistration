...
class ResUNet(nn.Module):
    def __init__(self, in_channel=1, out_channel=25 ,training=True):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        long_range5 = self.encoder_stage5(short_range3) + short_range4
        long_range5 = F.dropout(long_range5, self.dorp_rate, self.training)

        short_range5 = self.down_conv5(long_range5)

        outputs = self.decoder_stage1(long_range5) + short_range5
        outputs = F.dropout(outputs, self.dorp_rate, self.training)

        output1 = self.map1(outputs)

        short_range7 = self.up_conv1(outputs)

        outputs = self.decoder_stage2(torch.cat([short_range7, long_range4], dim=1)) + short_range7
        outputs = F.dropout(outputs, self.dorp_rate, self.training)

        output2 = self.map2(outputs)

        short_range8 = self.up_conv2(outputs)

        outputs = self.decoder_stage3(torch.cat([short_range8, long_range3], dim=1)) + short_range8
        outputs = F.dropout(outputs, self.dorp_rate, self.training)
        ...
        outputs = self.decoder_stage4(torch.cat([short_range9, long_range2], dim=1)) + short_range9

        output4 = self.map4(outputs)

        short_range10 = self.up_conv4(outputs)

        outputs = self.decoder_stage5(torch.cat([short_range10, long_range1], dim=1)) + short_range10
