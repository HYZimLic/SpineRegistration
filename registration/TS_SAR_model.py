...
class TS_SAR_stage(nn.Module):
    def __init__(self, img_size=128, patch_size=[3, 7, 15], stride=[2, 4, 8], num_classes=6, embed_dims=[256, 256, 256],
    ...
    def forward(self, x, y, x_label, y_label):
        ...
        for i in range(self.num_stages):
            if i == 0:
                ...
                xy = torch.cat([x_label, y_label], dim=1)
            ...
            if i < self.num_stages - 1:
                ...
                warpped_x, affm = self.affine_transform(x_label, affine)
                warpped_x_list.append(warpped_x)

        return warpped_x_list, y_label, affine_list, affm


class AffineCOMTransform(nn.Module):
    def __init__(self, use_com=False):
    ...
    def forward(self, x, affine_para):
        ...
        if self.use_com:
            ...
            _, _, width, height, depth = x.shape
            center_mass_x = width / 2
            center_mass_y = height / 2
            center_mass_z = depth / 2
        ...
        trans_xyz = affine_para[0, 0:3] * 20
        rotate_xyz = affine_para[0, 3:6] *180 * math.pi
        ...
        #                                                                                              self.translation_m)))))
        output_affine_m = torch.mm(to_center_matrix, torch.mm(self.translation_m,torch.mm(reversed_to_center_matrix, self.rotation_m)))
        affine_m = output_affine_m.cpu().detach().numpy()
        x = x.squeeze().cpu().detach().numpy()
        transformed_x = scipy.ndimage.affine_transform(x, affine_m)
        transformed_x[transformed_x < 0] = 0
        transformed_x = np.reshape(transformed_x, (1,1,) + transformed_x.shape)
        ...
        return torch.from_numpy(transformed_x).float().to('cuda:0'), output_affine_m
