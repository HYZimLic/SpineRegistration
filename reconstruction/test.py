...
def main():
    ...
    class MedReconDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            volume = np.zeros((args.output_size, args.output_size, args.output_size), dtype=np.uint8)
            ### load image
            ...
            ### load target
            for view_idx in range(args.num_views):
                volume_path = os.path.join(exp_path, 'data/3D_CT_{}.nii.gz'.format(view_idx + 1))
    ...
    # test evaluation 
    loss, pred_data, mid_data = test(test_loader, model, criterion, mode='Test')

    # show image
    for idx in range(args.test):
        save_path = os.path.join(exp_path, 'result/sample_{}'.format(idx + 1))
        savemid_path = os.path.join(midd_path, 'result/sample_{}'.format(idx + 1))
        ...
        imageSave(mid, groundtruth, args.vis_plane, savemid_path)
    return 
...
def test(val_loader, model, criterion, mode):
    model.eval()
    losses = AverageMeter()
    mid = np.zeros((args.test, args.middle_channel, args.middle_size, args.middle_size), dtype=np.float32)
    ...
    np.savez(file_mid_name, mid=mid)
    np.savez(file_name, pred=pred)

    return losses.avg, pred, mid
